...
(() => {
  ...
  const COLLISION_TINT_ENABLED = false; // désactive le tint lors des collisions (évite la boucle)
  ...
  function buildHalfSVG(rect, part){
    ...
    t.textContent = (part === 0 ? rect.termA : rect.termB);
  ...
  function buildRectSVG(rect){
    ...
    lbl.textContent = `${rect.termA} / ${rect.termB}`;
  ...
  const overlapPairs = new Set(); // pour déclencher le tint une seule fois par contact
  ...
  function enforceCollisions(){
    // Superposition autorisée : pas de repoussement.
    // Tint collision : déclenché UNE SEULE FOIS à l'entrée en contact, et désactivé par défaut.
    ...
    // cleanup (in case elements removed)
  ...
  function snapTargetsFor(el){
    ...
    const s = getSize(el); // same size for both halves
    ...
    if (!mobile){
      // pair in height: part0 above part1
      if (part === 0){
        // moving top -> align above other
        wantX = pOther.x;
        wantY = pOther.y - s.h;
      } else {
        // moving bottom -> align below other
        wantX = pOther.x;
        wantY = pOther.y + s.h;
      }
    } else {
    ...
    return [{ target: other, wantX, wantY, good:true }];
  }
  ...
  function snapAssemble(movedEl, otherEl, wantX, wantY){
    ...
    // snap animation for moved piece
