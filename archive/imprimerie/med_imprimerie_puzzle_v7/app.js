...
(() => {
  ...
  // ===== Config =====
  const SHOW_RECT_BADGES = false; // RX hidden on formed pairs by default

  const LEVEL_COUNT = 8; // number of rectangles
  const SNAP_DIST = 36;
  ...
  const COLLIDE_TINT_COOLDOWN = 120; // ms

  // allow overlap up to 1/3 of the smaller area
  const MAX_OVERLAP_RATIO = 1/3;
  ...
  class AudioManager{
    constructor(){
      ...
      // Browsers require a gesture to start audio. We'll arm on first pointer.
      const arm = () => {
        window.removeEventListener("pointerdown", arm);
        // do nothing yet; user can toggle music
      };
    ...
    async toggleMusic(){
      ...
      try{
        ...
        // ignore autoplay issues; user can click again
      }
  ...
  // ===== Symbols (8) =====
  // draw(svg, x, y, size, opacity)
  // We keep a simple symbol set; pad uses same.
  const SYMBOLS = [
  ...
  // - id: R1..R8
  // - leftKey: null or 1..8
  // - rightKey: null or 1..8
  // - sym: 0..7
  const level = [];
  for (let i=0;i<LEVEL_COUNT;i++){
    level.push({
      id: `R${i+1}`,
      label: ["SCELLES","Caches","Tampons","Encrage","Matrices","Nettoyage","Contrôle","Dossier"][i] || `Module ${i+1}`,
      leftKey: i===0 ? null : i,          // C1..C7 as left sockets
      rightKey: i===LEVEL_COUNT-1 ? null : i+1, // C1..C7 as right plugs (matches next)
      sym: i, // unique symbol per rect for now
    });
  }

  // ===== SVG Builders =====
  function buildHalfSVG(rect, part){
    // part 0 = top(Desktop)/left(Mobile) with male tab; part 1 = bottom/right with female hole
    ...
    const internalType = `C${rect.leftKey || rect.rightKey || 1}`; // internal connector per rectangle (for pairing halves)
    ...
    // seal
    content.appendChild(svgEl("rect", { x:HALF_W/2-50, y:10, width:100, height:24, rx:8, fill:"none", stroke:"rgba(156,47,47,.55)", "stroke-width":2, opacity:.65 }));
    const st = svgEl("text", { x:HALF_W/2, y:27, "text-anchor":"middle", fill:"rgba(156,47,47,.60)", "font-size":12, "font-weight":900 });
    st.textContent = "NOEL OK";
  ...
  function buildRectSVG(rect){
    ...
    // left side = female hole if applicable
    let maskId = null;
    let holeStroke = null;
    ...
    content.appendChild(svgEl("rect", { x:bandX, y:bandY, width:bandW, height:bandH, rx:13, fill:"rgba(0,0,0,.10)" }));
    const lbl = svgEl("text", { x:rectW/2, y:bandY+18, "text-anchor":"middle", fill:"rgba(0,0,0,.78)", "font-size":12, "font-weight":900 });
    lbl.textContent = rect.label;
    ...
    // reveal rectangle grows vertically (CSS anim on .reveal)
    ...
    // mild emboss effect via double draw
    const symG = svgEl("g");
    ...
    // right side male tab if applicable
    if (!mobile){
      if (rect.rightKey != null){
  ...
  // ===== Piece State =====
  // Pieces are DOM elements with dataset:
  // - kind: "half" or "rect"
  // - rid: rect id
  // - part: 0/1 for half
  // - internal: Cx for pairing halves
  // - leftKey/rightKey: for chaining (rect only)
  // - sym: 0..7
  // We also keep physics state in a Map.
  ...
  function separate(elA, elB){
    const a = rectFromEl(elA);
    const b = rectFromEl(elB);
    ...
    const pushX = iw < ih;
    const sa = phys.get(elA), sb = phys.get(elB);
    if (!sa || !sb) return;

    const centerAx = a.x + a.w/2, centerBx = b.x + b.w/2;
    const centerAy = a.y + a.h/2, centerBy = b.y + b.h/2;
    const dirX = centerAx < centerBx ? -1 : 1;
    const dirY = centerAy < centerBy ? -1 : 1;

    const strength = 0.55;
    if (pushX){
      const dx = (iw * strength) * dirX;
      if (!sa.dragging) sa.x += dx;
      if (!sb.dragging) sb.x -= dx;
      sa.vx += dx * 0.015;
      sb.vx -= dx * 0.015;
    } else {
      const dy = (ih * strength) * dirY;
      if (!sa.dragging) sa.y += dy;
      if (!sb.dragging) sb.y -= dy;
      sa.vy += dy * 0.015;
      sb.vy -= dy * 0.015;
    }

    setPos(elA, sa.x, sa.y);
    setPos(elB, sb.x, sb.y);
    clampInside(elA);
    clampInside(elB);
  }
  ...
  function snapTargetsFor(el){
    ...
    // Only snap halves together of same rid and opposite part.
    const otherPart = part === 0 ? 1 : 0;
    const target = Array.from(phys.keys()).find(o =>
      o !== el &&
      o.dataset.kind === "half" &&
      o.dataset.rid === rid &&
    ...
    // Determine anchor points (internal connector edges)
    const a = rectFromEl(el);
    const b = rectFromEl(target);

    if (!mobile){
      // pair in height: part0 goes above part1
      const wantX = b.x; // align left edges
      const wantY = b.y - a.h;
      return [{ target, wantX, wantY, good:true }];
    } else {
      // pair in width: part0 left of part1
      const wantX = b.x - a.w;
      const wantY = b.y;
      return [{ target, wantX, wantY, good:true }];
    }
  ...
  function badSnapNearby(el){
    ...
    // find any half close to where an attach could happen but wrong
    const a = rectFromEl(el);
    ...
    for (const c of candidates){
      ...
      // approximate check: closeness of edge alignment
      if (!mobile){
        // look for top/bottom adjacency
        const dy1 = Math.abs((a.y + a.h) - b.y);   // el bottom to other top
        const dy2 = Math.abs((b.y + b.h) - a.y);   // other bottom to el top
        const dx = Math.abs(a.x - b.x);
        const dy = Math.min(dy1, dy2);
        const d = Math.hypot(dx, dy);
        if (d < BAD_SNAP_DIST){
          // wrong if not exact matching pair
          const good = (el.dataset.rid === c.dataset.rid) && (el.dataset.internal === c.dataset.internal) && (Number(c.dataset.part) !== part);
          if (!good){
            if (!best || d < best.d) best = { other:c, d };
          }
        }
      } else {
  ...
  function snapAssemble(movedEl, otherEl, wantX, wantY){
    ...
    // determine which is part0/part1
    ...
    // animate movedEl to target position (so that moved aligns relative to other)
    const movedIsPart0 = movedEl === part0El;
    const targetX = movedIsPart0 ? wantX : getPos(part0El).x;
    const targetY = movedIsPart0 ? wantY : getPos(part0El).y;

    // If moved is part1, we want it to align so part0 is at current? easier:
    // We'll compute final positions for both:
    const mobile = isMobileLayout();
    const p1 = getPos(part1El);
    const p0Final = mobile ? { x: p1.x - getSize(part0El).w, y: p1.y } : { x: p1.x, y: p1.y - getSize(part0El).h };
    const p1Final = { x: p1.x, y: p1.y };

    const a0 = phys.get(part0El), a1 = phys.get(part1El);
    if (a0){ a0.vx = 0; a0.vy = 0; }
    if (a1){ a1.vx = 0; a1.vy = 0; }

    // animate both to their final "snap" positions
    animateTo(part0El, p0Final.x, p0Final.y, 160);
    animateTo(part1El, p1Final.x, p1Final.y, 160, () => {
      // Replace two halves by one rectangle
  ...
  function animateTo(el, x, y, ms=160, onDone=null){
    const s = phys.get(el);
  ...
  // ===== Line assembly (chain) =====
  function readChain(){
    // Find a chain by following connectors from START to END among rect pieces.
    const rects = Array.from(phys.keys()).filter(el => el.dataset.kind === "rect");
    if (!rects.length) return [];

    // Build maps from leftKey/rightKey
    const leftMap = new Map(); // key -> element (female side expects key)
    const rightMap = new Map(); // key -> element (male side provides key)

    for (const el of rects){
      const lk = el.dataset.leftKey ? Number(el.dataset.leftKey) : null;
      const rk = el.dataset.rightKey ? Number(el.dataset.rightKey) : null;
      if (lk != null) leftMap.set(lk, el);
      if (rk != null) rightMap.set(rk, el);
    }

    // Start is the one with leftKey null
  ...
  let unlockedSequence = null; // array of symbol ids once line is complete
  ...
  function attachDrag(el){
    ...
    el.addEventListener("pointerup", (ev) => {
      ...
      // attempt snap if half
      const snapped = trySnap(el);
      if (!snapped){
        setStatus("…");
      }
    });
  ...
  function stepPhysics(){
    for (const [el, s] of phys.entries()){
      ...
      // move by velocity (vx/vy in px/ms)
  ...
  function spawnPieces(){
    ...
    for (const rect of level){
      const internalType = `C${rect.leftKey || rect.rightKey || 1}`;
      ...
      for (let part=0; part<2; part++){
        ...
        el.dataset.internal = internalType;
        el.dataset.sym = String(rect.sym);
        ...
        // random position
        const x = Math.random() * (bounds.w - HALF_W);
        const y = Math.random() * (bounds.h - HALF_H);
  ...
  btnTheme.addEventListener("click", () => {
    munich = !munich;
    setThemeMunich(munich);
  });

  btnMusic.addEventListener("click", () => audio.toggleMusic());
  btnReset.addEventListener("click", reset);

  // Handle resize: easiest is full reset to keep geometry consistent
  let resizeT = null;
